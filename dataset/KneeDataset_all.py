...
def get_basedir(opt):
    return os.path.join(opt.data_dir, "Knee")


class KneeDataset(torch_data.Dataset):
    ...
    def __read_dataset(self):
        ...
        # with Pool() as p:
        #     p.map(self.load_nii_img, list(self.all_img_list.keys()))
        #     p.map(self.load_label, list(self.all_seg_list.keys()))
        #
    ...
    def parallel_read_dataset(self, l):
        ...
        img = self.all_img_list[itm["img_fid"]]
        lb  = self.all_seg_list[itm['lbs_fid']]
    ...
    # try:
        ...
        self.all_img_list[itm["img_fid"]] = img
        self.all_seg_list[itm['lbs_fid']] = lb

        if (idx + 1) % (length // 10) == 0:
        ...
        out_list = self.add_to_list(out_list, img, lb,
                                    sam, mean, std, _domain, scan_id, itm["img_fid"], itm['lbs_fid'])
    ...
    def get_slice(self, array, slices, is_start=False, is_end=False):
        # H, W, C
        arr = array[..., slices]

        if self.input_window == 3 and is_start:
            arr[..., 1] = arr[..., 0]
        if is_end:
            arr[..., 0] = arr[..., 1]
    ...
    def add_to_list(self, out_list, img, lb, sam, mean, std, _domain, scan_id, file_id, lbs_fid):
        ...
        for ii in range(1, img.shape[-1] - 1):
            if self.input_window == 3:
                slice = list(range(ii -1, ii + 2))   #img[..., ii -1: ii + 2].copy()
            elif self.input_window == 1:
                slice = [ii]                          #img[..., ii: ii + 1].copy()
            ...
            if self.fake_interpolate and _domain == "CHAOST2"  and self.is_train and False:    #
        ...
        if self.input_window == 3:
            slice =  list(range(ii-2, ii + 1))   #img[..., ii-2: ii + 1].copy()
    ...
    def __getitem__(self, index):
        ...
        lbs_fid = curr_dict["lbs_fid"]


        img_all = self.all_img_list[file_id]
        seg_all = self.all_seg_list[lbs_fid]

        # array, slices, is_start=False, is_end=False):

        img  = self.get_slice(img_all, curr_dict["img"],
                              is_start= curr_dict['is_start'], is_end=curr_dict['is_end'])            # H, W, C, [0 - 255]
        mask = self.get_slice(seg_all, curr_dict["lb"])     # H, W, C
...
def get_training(opt, modality, idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, pseudo = False):
    ...
    return KneeDataset(opt, idx_pct = idx_pct,
